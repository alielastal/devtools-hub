...
export default function UrlEncoder() {
  const { input, setInput, output, mode, setMode, error, process, encodeFullUrl, swap } = useUrlEncoder()

  return (
    <div className="flex h-full flex-col gap-4 p-4">
      <div>
        <h1 className="text-xl font-bold text-surface-900 dark:text-white">URL Encoder / Decoder</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {mode === 'encode' && (
          <button
            ...
            disabled={!input.trim()}
            className="rounded-lg border border-primary-300 bg-primary-50 px-4 py-1.5 text-sm font-medium text-primary-700 hover:bg-primary-100 disabled:opacity-50 dark:border-primary-700 dark:bg-primary-900/30 dark:text-primary-300 dark:hover:bg-primary-900/50"
          >
            Encode Full URL
          </button>
        )}
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <div className="flex flex-col">
          <span className="mb-1 text-xs font-semibold uppercase tracking-wider text-surface-400">Input</span>
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder={mode === 'encode' ? 'Enter text or URL to encode...' : 'Paste encoded URL...'}
            spellCheck={false}
        ...
        <div className="flex flex-col">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-surface-400">Output</span>
